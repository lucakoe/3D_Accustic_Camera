...
def calc_micpos_with_voc(data_dir, SEG, P, calibfile=None, h5f_outpath=None):
    ...
    with open(fpath_dat, 'rb') as fp_dat:
        ...
        R = scipy.optimize.minimize(get_error, x0=dx0, args=(P, SEG, data_dir, mic0pos, speedOfSound),
                                    method='L-BFGS-B', bounds=b, callback=callbackF)
...
def calc_vm_stats(data_dir, calibfile, roi=None, iter_id=None):
    ...
    def get_power_at_vm(data_dir, calibfile, seg2, fs, n_ch, pressure_calib, D, snout_pos, roi, speedOfSound,
                        stft_data):
        ...
        with open(fpath_dat, 'rb') as fp_dat:
            for i_ss in tqdm(range(np.max(I_ss) + 1)):
                ...
                result = joblib.Parallel(n_jobs=-1)(
                    joblib.delayed(get_power_at_vm)(data_dir, calibfile, seg2, fs, n_ch, pressure_calib, D,
                                                    snout_pos[i_frame, :], roi, speedOfSound, stft_data) for i_trial in
                    range(n_trial))
...
def detect_bbv(data_dir, thr=1.4):
    ...
    def fun(x, t_ori, intv):
        ...
        xx = x[I]
        ff, tt, ss = scipy.signal.spectrogram(xx, fs, window=('hamming'), nperseg=int(fs / 100),
                                              noverlap=int(fs / 100 * 0.9))
    ...
    with open(fpath_dat, 'rb') as fp_dat:
        ...
        for i_frame in tqdm(range(n_frame)):
            ...
            if t[i_frame] - pre_t >= 1:
                ...
                dt = 0.0005
                fftsize = 512
                clrs = [(255, 255, 0), (255, 0, 255), (0, 255, 255), (0, 255, 0), (0, 128, 255), (0, 0, 255),
                        (255, 0, 128)]
                ...
                if mask is not None:
                    for intv in mask:
                        if intv[2] == 1:
                            ...
                            cv2.rectangle(spec_show_masked, (int(x1), 0), (int(x2), spec_show.shape[0]), (0, 0, 50),
                                          thickness=-1)
                    ...
                    for intv in mask:
                        if intv[2] == 2:
                            x1 = (intv[0] - t_intv[0]) / (t_intv[1] - t_intv[0]) * spec_show.shape[1]
                            x2 = (intv[1] - t_intv[0]) / (t_intv[1] - t_intv[0]) * spec_show.shape[1]
                            cv2.rectangle(spec_show_masked, (int(x1), 0), (int(x2), spec_show.shape[0]), (0, 0, 0),
                                          thickness=-1)
            ...
            for i_mouse in range(n_mice):
                if np.sum(np.isnan(sp[i_mouse, :])) == 0:
                    cv2.circle(frame, (int(sp[i_mouse, 0]), int(sp[i_mouse, 1])), 3, clrs[i_mouse], thickness=-1,
                               lineType=cv2.LINE_AA)
                    if np.sum(seg2[:, 1] == i_mouse) > 0:
                        cv2.circle(frame, (int(sp[i_mouse, 0]), int(sp[i_mouse, 1])), 20, clrs[i_mouse], thickness=2,
                                   lineType=cv2.LINE_AA)
...
def estimate_assign_param(data_dirs, calibfiles, outfile, n_trial=7, iter_ids=None, show_figs=True):
    with h5py.File(outfile, mode='w') as f_out:
        ...
        if show_figs:
            print(
                'distance between snout and peak, 75, 95 and 99 percentile: {:.2f} deg, {:.2f} deg, {:.2f} deg'.format(
                    rad2deg(np.percentile(Da, 75)), rad2deg(d95), rad2deg(d99)))
            print('median = {:.2f} deg'.format(rad2deg(np.percentile(Da, 50))))
            plt.hist(rad2deg(Da), bins=500, range=[0, 40])
            plt.xlabel('error (deg)')
            plt.show()

            print('in px, 75, 95 and 99 percentile: {:.2f} px, {:.2f} px, {:.2f} px'.format(np.percentile(Dp, 75),
                                                                                            np.percentile(Dp, 95),
                                                                                            np.percentile(Dp, 99)))
                    ...
                    with h5py.File(vmstatfile, mode='r') as f:
                        ...
                        for i_k, k in enumerate(tqdm(keys)):
                            ...
                            for i_d in range(D.shape[0] - 1):
                                _, p = scipy.stats.wilcoxon(r2[:, i_d + 1, I[i_d + 1, 0]],
                                                            r2[:, i_d + 1, I[i_d + 1, 1]])
...
def locate_all_segs(data_dir, calibfile, vid_mrgn=100, roi=None, out_sspec=False, color_eq=False):
    ...
    with open(fpath_dat, 'rb') as fp_dat:
        for i_ss in tqdm(range(np.max(I_ss) + 1)):
            ...
            S, peaks = locate_seg(fp_dat, seg2, tau, grid_shape, fs, n_ch, pressure_calib, vid_size, vid_mrgn=vid_mrgn,
                                  roi=roi)
